module mod_cuda
  ...
  public :: cuda_matrix_clustering  ! Fortran wrapper for complete GPU clustering
  ...
  interface
    ...
    function cuda_matrix_clustering_c(h_matrix, h_active_points, h_cluster_size, &
        h_cluster_parent, h_cluster_count, h_active_clusters, n, dist_threshold, linkage_type) &
        bind(C, name='cuda_matrix_clustering_c')
      use iso_c_binding
      implicit none
      type(c_ptr), value :: h_matrix
      type(c_ptr), value :: h_active_points
      type(c_ptr), value :: h_cluster_size
      type(c_ptr), value :: h_cluster_parent
      type(c_ptr), value :: h_cluster_count
      type(c_ptr), value :: h_active_clusters
      integer(c_int), value :: n
      real(c_double), value :: dist_threshold
      integer(c_int), value :: linkage_type
      integer(c_int) :: cuda_matrix_clustering_c
    end function cuda_matrix_clustering_c
...
contains
  ...
  !! @param[in]     linkage_str         Linkage type ('min', 'max', or 'mean')
  !! @param[in,out] cluster_parent      Output cluster parent for each point (n) - O(n) tracking
  ...
  function cuda_matrix_clustering(matrix, n, dist_threshold, linkage_str, &
                                     cluster_parent, cluster_count, active_points, cluster_size) result(status)
    ...
    cuda_status = cuda_matrix_clustering_c( &
      c_loc(matrix(1, 1)), &
      c_loc(c_active_in(1)), &
      c_loc(cluster_size(1)), &
      c_loc(cluster_parent(1)), &
  ...
  end function cuda_matrix_clustering
