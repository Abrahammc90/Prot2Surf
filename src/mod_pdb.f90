...
!! expanded for the clustering tools in this workspace. Notable
!! enhancements include the addition of `count_residues` and
!! `count_chains` functions and changes that allow creation of richer
!! in-memory objects where residues contain their atoms and chains
!! contain residues and atoms. These changes make it possible to build
!! `type_pdb_file` instances with proper `residues` and `chains`
!! substructures (each storing their atoms) rather than storing atoms
!! in a single flat list only.
!!
!! The original SDA7 code and author attribution are retained; consult
...
module mod_pdb
 ...
 contains
   ...
   !!
   !! This routine allocates the storage for `this%atoms`, `this%residues`
   !! and `this%chains` using the supplied counts and performs a
   !! preliminary scan of the open Fortran unit `pdb_file` to determine
   !! per-chain and per-residue allocation sizes for nested arrays.
   !!
   !! @param[in,out] this     `type_pdb_file` instance to allocate and initialize
   !! @param[in]     pdb_file Fortran unit number of an already-open PDB file
   !! @param[in]     natoms   Total number of ATOM/HETATM records expected
   !! @param[in]     nresidues Number of residues expected
   !! @param[in]     nchains  Number of chains expected
   ...
   !!
   !! Scans the open Fortran file unit `pdb_file` and returns the number
   !! of lines starting with `ATOM` or `HETATM`.
   !!
   !! @param[in] pdb_file Fortran unit number of an already-open PDB file
   !! @return natoms     Number of ATOM/HETATM records found
   ...
   !!
   !! Scans the open Fortran file unit `pdb_file` and counts residue
   !! changes by comparing residue identifiers in ATOM/HETATM records.
   !! Returns the number of distinct residues found.
   !!
   !! @param[in] pdb_file Fortran unit number of an already-open PDB file
   !! @return nresidues  Number of residues found
   ...
   !!
   !! Scans the open Fortran file unit `pdb_file` and counts the number
   ...
   !!
   !! Reads ATOM/HETATM records from the already-open Fortran unit
   !! `pdb_file`, fills `this%atoms` with atom records and populates
   !! `this%residues` and `this%chains` so that residues contain their
   !! per-residue `atoms` arrays and chains contain their residues and
   !! atoms. The routine expects `this` to have been allocated with
   !! the correct sizes (see `allocate_pdb_object`).
   !!
   !! @param[in,out] this     Pre-allocated `type_pdb_file` to populate
   !! @param[in]     pdb_file Fortran unit number of an already-open PDB file
   subroutine fill_pdb_object (this, pdb_file)
    ...
    ! Variables
    integer :: chain_atom_i, chain_residue_i, residue_atom_i
