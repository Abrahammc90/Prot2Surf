!> \file contact_map.f90
!! \brief Residue proximity analysis across transformed encounters.
!!
!! contact_map.f90 - Find residues close to the surface across encounters.
!!
!! Usage: ./contact_map -help
!!
  ...
  ! 5. Write results to `closest_residues.txt`
    ...
    do while ( count_arg <= nb_argument )
        ...
        if ( trim(argument) == "-pdb2" ) then
          ...
          if (ios /= 0) then
            print *, '[contact_map] ERROR: Integer expected for the -nb_encounters argument.'
          end if
          count_arg = count_arg + 1
            ...
            if (ios /= 0) then
              print *, '[contact_map] ERROR: Real expected for the -threshold argument.'
            end if
            count_arg = count_arg + 1
        else if ( trim(argument) == "-help" ) then
          help_bool = .true.
        else
          print *, '[contact_map] ERROR: Argument ''', trim(argument), ''' not recognized.'
          print *, '[contact_map] For usage, run: ./contact_map -help'
          STOP 1
        end if
    ...
    if (.not. pdb2_bool) then
      print *, '[contact_map] ERROR: PDB file not provided. Use -pdb2 <file>'
      print *, '[contact_map] For usage, run: ./contact_map -help'
      STOP 1
    end if

    if (.not. complexes_bool) then
      print *, '[contact_map] ERROR: Complexes file not provided. Use -complexes <file>'
      print *, '[contact_map] For usage, run: ./contact_map -help'
      STOP 1
    end if
    ...
    if ( nb_encounters .gt. tot_encounters ) then
        print *, '[contact_map] WARNING: -nb_encounters > available. Using all encounters in ', trim(complexes_filename)
        nb_encounters = tot_encounters
        print *, '[contact_map] Total encounters available: ', nb_encounters
      else if (.not. nb_encounters_bool) then
        print *, '[contact_map] WARNING: -nb_encounters not given. Using all available.'
        nb_encounters = tot_encounters
        print *, '[contact_map] If you want a specific number, use -nb_encounters <N>'
      end if
    ...
    contains
    !> Print usage/help text for `contact_map`.
    !!
    ...
    subroutine print_help()
      ...
      print *, ''
       print *, '[contact_map] Usage: ./contact_map -pdb2 <pdbfile> -complexes <file> [-nb_encounters N] [-threshold X]'
       print *, '[contact_map] Example: ./contact_map -pdb2 p2_noh.pdb ', &
           '-complexes assoc_complexes -nb_encounters 5000 -threshold 6.0'
       print *, '[contact_map] Defaults: nb_encounters=all, threshold=6.0'
      STOP
